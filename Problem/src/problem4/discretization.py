"""
Discretization Module for Problem 4
=====================================

Converts continuous generation variables to binary variables for QUBO formulation.
Reuses logic from Problem 3 but adapted for reduced problem size.
"""
...
class GenerationDiscretizer:
    ...
    def __init__(self, P_min: np.ndarray, P_max: np.ndarray, num_bits: int = 1):
        """
        ...
        Args:
            ...
            num_bits: Number of bits per unit per period (default: 1)
        """
        self.P_min = P_min
        self.P_max = P_max
    ...
    def get_generation_value(self, i: int, t: int, binary_vars: Dict[str, int]) -> float:
        ...
        if self.num_bits == 1:
            ...
            var_name = self.get_variable_name(i, t, 0)
            ...
            p_int = 0
            for k in range(self.num_bits):
                var_name = self.get_variable_name(i, t, k)
                bit_value = binary_vars.get(var_name, 0)
                p_int += bit_value * (2**k)
